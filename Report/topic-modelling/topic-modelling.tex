...
what we did in \Sectionref{sec:classification-preprocessing}. In summary, we perused our tweets
...
twitter follow a specific pattern so they can be easily removed with a regular expression. Secondly,
our dataset is Apple centric so words like ``iphone'', ``ipod'' and ``ipad'' are bound to ocur in
almost every tweet. For this reason, we add these words to our stop words list and remove them from
every tweet. The regular expression used can be found in
Appendix~\ref{sec:appx-regular-expressions}.
...
In \Sectionref{sec:evaluating-30-topics}, we analysed a topic model that generated 30 topics on our
