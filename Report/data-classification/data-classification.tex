...
the mean value. Comparing the highest AUC in both tables(values in bold), we can see that the removal
of stop words gives us approximately 2.2\%(from 0.7455 to 0.7618) increase in AUC\@.
Figures~\ref{fig:auc-with-stopwords} and~\ref{fig:auc-without-stopwords} gives a visual
representation of the AUC curve. This increase proves that training a classifier without stop words
in our dataset gives better performance.
...
necessarily 5 times relevant that a document with only 1 occurrence of the same word. This is where
...
0.76AUC in the previous section and our highest value after using the tf-idf weighting scheme is
approximately 0.80AUC which is a 5.7\% increase in AUC\@. \Figref{fig:auc-tfidf} is a visual
representation of the area under the ROC curve, AUC.
...
% It is usually composed of the term frequency and the inverse document frequency. The term frequency is the number of times a word appears in 
